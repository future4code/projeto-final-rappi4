...
import { PageContainer, StyledParagraph } from './styled'
...
export default class SearchPage extends Component {
    ...
    searchRestaurants = (string, restaurantList) => {
        if (string.length) {
            const condition = new RegExp("(" + string.toLowerCase() + ")", "g");
            return restaurantList.filter(restaurant => condition.test(restaurant.name.toLowerCase()));
        }
    ...
    render() {
        ...
        let searchResult = this.searchRestaurants(searchString, mockRestaurants);

        if (searchResult.length) {
            searchResult = searchResult.map((result, i) => <RestaurantCard key={i} restaurant={result} />);
        }
        else {
            searchResult = <StyledParagraph>Não encontramos :(</StyledParagraph>
        }
        return (
            <PageContainer>
